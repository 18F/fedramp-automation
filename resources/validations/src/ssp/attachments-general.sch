<sch:pattern xmlns:sch="http://purl.oclc.org/dsdl/schematron">
    ...
    <sch:rule context="oscal:back-matter/oscal:resource">
        ...
        <sch:assert role="error" test="@uuid">
            A &lt;
            <sch:name />
            &gt; element must have a uuid attribute
        </sch:assert>
        <!--<sch:assert
                role="error"
        ...
        <sch:assert id="resource-has-title" role="warning" test="oscal:title">
            &lt;
            <sch:name />
            uuid="
            <sch:value-of select="@uuid" />
            "&gt; SHOULD have a title
        </sch:assert>
        <sch:assert id="resource-has-rlink" role="error" test="oscal:rlink">
            &lt;
            <sch:name />
            uuid="
            <sch:value-of select="@uuid" />
            "&gt; must have an &lt;rlink&gt; element
        </sch:assert>
        <sch:assert id="resource-is-referenced" role="info" test="$INFO and @uuid = (//@href[matches(., '^#')] ! substring-after(., '#'))">
            <sch:value-of select="$path" />
            has no reference within the document
        </sch:assert>
    </sch:rule>
    <sch:rule context="oscal:back-matter/oscal:resource/oscal:prop[@name = 'type']">
        <sch:assert id="attachment-type-is-valid" test="@value = $attachment-types">
            Found unknown attachment type «
            <sch:value-of select="@value" />
            » in
            <sch:value-of select="
    ...
    <sch:rule context="oscal:back-matter/oscal:resource/oscal:rlink">
        <sch:assert id="rlink-has-href" role="error" test="@href">
            A &lt;
            <sch:name />
            &gt; element must have an href attribute
        </sch:assert>
