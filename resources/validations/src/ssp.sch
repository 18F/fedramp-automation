<sch:schema queryBinding="xslt2" xmlns:o="http://csrc.nist.gov/ns/oscal/1.0" xmlns:sch="http://purl.oclc.org/dsdl/schematron" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xpath-default-namespace="http://csrc.nist.gov/ns/oscal/1.0">
    ...
    <sch:let name="WARNING" value="false()" />
    <!-- gratuitous info off by default -->
    <sch:let name="INFO" value="false()" />
    ...
    <xsl:function as="document-node()*" name="lv:profile">
        ...
        -->
            <profile href="https://raw.githubusercontent.com/usnistgov/oscal-content/0f78f05f0953e64f37b5cb24e0522030d82fc1fa/fedramp.gov/xml/FedRAMP_LOW-baseline-resolved-profile_catalog.xml" level="low" />
            <profile href="https://raw.githubusercontent.com/usnistgov/oscal-content/0f78f05f0953e64f37b5cb24e0522030d82fc1fa/fedramp.gov/xml/FedRAMP_MODERATE-baseline-resolved-profile_catalog.xml" level="moderate" />
            <profile href="https://raw.githubusercontent.com/usnistgov/oscal-content/0f78f05f0953e64f37b5cb24e0522030d82fc1fa/fedramp.gov/xml/FedRAMP_HIGH-baseline-resolved-profile_catalog.xml" level="high" />
        </xsl:variable>
        ...
        <analysis>
            ...
            <reports count="{count($element)}" description="{$value-set/f:description}" formal-name="{$value-set/f:formal-name}" name="{$value-set/@name}">
                <xsl:for-each select="$ok-values">
                    ...
                    <report count="{count($match)}" value="{current()}"></report>
                </xsl:for-each>
            </reports>
        </analysis>
    </xsl:template>
    <xsl:template as="xs:string" name="report-template">
        ...
        <xsl:value-of>
            ...
            There are
            <xsl:value-of select="($analysis/reports/@count - sum($analysis/reports/report/@count))" />
            invalid items.
            <xsl:if test="count($analysis/errors/error) &gt; 0">
                ...
                <xsl:for-each select="$analysis/errors/error">
                    ...
                    , so analysis could be inaccurate or it completely failed.
                </xsl:for-each>
            </xsl:if>
        </xsl:value-of>
    </xsl:template>
    <sch:pattern>
        <sch:rule context="/oscal:system-security-plan">
            ...
            <sch:assert id="no-registry-values" role="fatal" test="count($registry/f:fedramp-values/f:value-set) &gt; 0">
                The registry values at the path '
                <sch:value-of select="$registry-href" />
                ' are not present, this configuration is invalid.
            </sch:assert>
            <sch:assert id="no-security-sensitivity-level" organizational-id="section-c.1.a" role="fatal" test="$sensitivity-level != ''">[Section C Check 1.a] No sensitivty level found, no more validation processing can
                        occur.</sch:assert>
            <sch:assert id="invalid-security-sensitivity-level" organizational-id="section-c.1.a" role="fatal" test="empty($ok-values) or not(exists($corrections))">
                [Section C Check 1.a]
                <sch:value-of select="./name()" />
                is an invalid value of '
                <sch:value-of select="lv:sensitivity-level(/)" />
                ', not an allowed value of
                <sch:value-of select="$corrections" />
        ...
        <sch:rule context="/oscal:system-security-plan/oscal:control-implementation">
            ...
            <sch:report id="each-required-control-report" role="positive" test="count($required-controls) &gt; 0">
                ...
                <sch:value-of select="if (count($required-controls)=1) then ' control' else ' controls'" />
                are required:
                <sch:value-of select="$required-controls/@id" />
            </sch:report>
            <sch:assert id="incomplete-core-implemented-requirements" organizational-id="section-c.3" role="error" test="not(exists($core-missing))">
                [Section C Check 3] This SSP has not implemented the most important
                <sch:value-of select="count($core-missing)" />
                core
                <sch:value-of select="if (count($core-missing)=1) then ' control' else ' controls'" />
                :
                <sch:value-of select="$core-missing/@id" />
            </sch:assert>
            <sch:assert id="incomplete-all-implemented-requirements" organizational-id="section-c.2" role="warn" test="not(exists($all-missing))">
                [Section C Check 2] This SSP has not implemented
                <sch:value-of select="count($all-missing)" />
                <sch:value-of select="if (count($all-missing)=1) then ' control' else ' controls'" />
                overall:
                <sch:value-of select="$all-missing/@id" />
            </sch:assert>
            <sch:assert id="extraneous-implemented-requirements" organizational-id="section-c.2" role="warn" test="not(exists($extraneous))">
                [Section C Check 2] This SSP has implemented
                <sch:value-of select="count($extraneous)" />
                extraneous
                <sch:value-of select="if (count($extraneous)=1) then ' control' else ' controls'" />
        ...
        <sch:rule context="/oscal:system-security-plan/oscal:control-implementation/oscal:implemented-requirement">
            ...
            <sch:report id="implemented-response-points" role="positive" test="exists($implemented)">
                [Section C Check 2] This SSP has implemented a statement for each of the following lettered
                        response points for required controls:
                <sch:value-of select="$implemented/@statement-id" />
                .
            </sch:report>
            <sch:assert id="missing-response-points" organizational-id="section-c.2" role="error" test="not(exists($missing))">
                [Section C Check 2] This SSP has not implemented a statement for each of the following lettered
                        response points for required controls:
                <sch:value-of select="$missing/@id" />
                .
            </sch:assert>
        </sch:rule>
        <sch:rule context="/oscal:system-security-plan/oscal:control-implementation/oscal:implemented-requirement/oscal:statement">
            ...
            <sch:assert id="missing-response-components" role="warning" test="$components-count &gt;= $required-components-count">
                ...
                <sch:value-of select="if (count($components-count)=1) then ' component' else ' components'" />
                with a description. There are
                <sch:value-of select="$components-count" />
                .
            </sch:assert>
        </sch:rule>
        <sch:rule context="/oscal:system-security-plan/oscal:control-implementation/oscal:implemented-requirement/oscal:statement/oscal:description">
            <sch:assert id="extraneous-response-description" role="warning" test=". =&gt; empty()">
                Response statement
                <sch:value-of select="../@statement-id" />
                has a description not within a component. That was previously allowed, but not recommended. It
        ...
        <sch:rule context="/oscal:system-security-plan/oscal:control-implementation/oscal:implemented-requirement/oscal:statement/oscal:remarks">
            <sch:assert id="extraneous-response-remarks" role="warning" test=". =&gt; empty()">
                Response statement
                <sch:value-of select="../@statement-id" />
                has remarks not within a component. That was previously allowed, but not recommended. It will
        ...
        <sch:rule context="/oscal:system-security-plan/oscal:control-implementation/oscal:implemented-requirement/oscal:statement/oscal:by-component/oscal:description">
            ...
            <sch:assert id="incomplete-response-description" organizational-id="section-d" role="error" test="$description-length &gt;= $required-length">
                [Section D Checks] Response statement component description for
                <sch:value-of select="../../@statement-id" />
                is too short with
                <sch:value-of select="$description-length" />
                characters. It must be
                <sch:value-of select="$required-length" />
                characters long.
            </sch:assert>
        </sch:rule>
        <sch:rule context="/oscal:system-security-plan/oscal:control-implementation/oscal:implemented-requirement/oscal:statement/oscal:by-component/oscal:remarks">
            ...
            <sch:let name="remarks-length" value="$remarks =&gt; string-length()" />
            <sch:assert id="incomplete-response-remarks" organizational-id="section-d" role="warning" test="$remarks-length &gt;= $required-length">
                [Section D Checks] Response statement component remarks for
                <sch:value-of select="../../@statement-id" />
                is too short with
                <sch:value-of select="$remarks-length" />
                characters. It must be
                <sch:value-of select="$required-length" />
                characters long.
            </sch:assert>
        ...
        <sch:rule context="/oscal:system-security-plan/oscal:metadata">
            ...
            <sch:assert id="incorrect-role-association" organizational-id="section-c.6" test="not(exists($extraneous-roles))">
                ...
                <sch:value-of select="if (count($extraneous-roles)=1) then ' role' else ' roles'" />
                not defined in the role:
                <sch:value-of select="$extraneous-roles/@role-id" />
            </sch:assert>
            <sch:assert id="incorrect-party-association" organizational-id="section-c.6" test="not(exists($extraneous-parties))">
                [Section C Check 2] This SSP has defined a responsible party with
        ...
        <sch:rule context="/oscal:system-security-plan/oscal:back-matter/oscal:resource/oscal:base64">
            ...
            <sch:assert id="resource-base64-available" organizational-id="section-b.?????" test="./@filename">
                [Section B Check ????] This SSP has file name:
                <sch:value-of select="./@filename" />
            </sch:assert>
            <sch:assert id="resource-base64-available" organizational-id="section-b.?????" test="./@filename">
                [Section B Check ????] This SSP has media type:
                <sch:value-of select="./@media-type" />
            </sch:assert>
        </sch:rule>
    </sch:pattern>
