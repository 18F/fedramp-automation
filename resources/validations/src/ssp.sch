...
<sch:pattern>
    ...
    <sch:let name="registry" value="$registry-href => lv:registry()"/>
        ...
        <sch:assert id="incomplete-implementation-requirements" test="not(exists($all-missing))">This SSP has not implemented <sch:value-of select="count($all-missing)"/><sch:value-of select="if (count($all-missing)=1) then ' control' else ' controls'"/> overall: <sch:value-of select="$all-missing/@id"/></sch:assert>
